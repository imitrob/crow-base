...
from sensor_msgs.msg import CameraInfo, CompressedImage, Image  # noqa
...
CHECKERBOARD_SIZE = (9, 6)  # (width, height)
CHECKERBOARD_CELL_SIZE = 0.025  # in meters
...
def getTopicListBAndEdit(topic, parent):
    ...
    #print(node_names_namespaces)
    #node.get_publisher_names_and_types_by_node('camera','/')
    for each_node in node_names_namespaces:
        try:
            publishers = node.get_publisher_names_and_types_by_node(each_node[0], each_node[1])
            #print(publishers)
            for p in publishers:
            ...
            print('NodeNameNonExistentError for node ' + each_node)
    print('topic list: ')
    print(topics)
    #for t in topic:
    #    topics += rostopic.find_by_type(t)
        #topic_list = self.node.get_publisher_names_and_types_by_node(node_name, namespace)
...
def quat2mat(quat_ros):
    quat = [getattr(quat_ros, a) for a in "xyzw"]

    rot = np.zeros((4, 4))
    rot[:3, :3] = tf3.quaternions.quat2mat(quat[3:] + quat[:3])
    rot[3, 3] = 1
    return rot
...
class PoseEstimator():
    ...
    def __init__(self, cameraMatrix, distCoeffs, node):
    ...
    def estimateBoardPose(self, image, init_rvec=None, init_tvec=None):
        ...
        # markerCorners, markerIds, rejectedPts = cv2.aruco.detectMarkers(image, self.dictionary)
        ...
        if markerIds is not None:
            if len(markerIds) >= 2:
                ...
                self.node.get_logger().info("Only one marker detected, origin pose might be incorrect!")
            ...
            # self.node.get_logger().info("Cannot detect markers, calibration chessboard might be detected with a wrong orientation!")
            pass
        ret, corners = cv2.findChessboardCorners(image, (self.BOARD_COLS, self.BOARD_ROWS), flags=self.chessFlags)
        # print(ret, corners)
        # print(ret)
        # ret = not (np.any(np.isnan(corners)) or len(corners) == 0)
        # print(ret, corners)
        # print(ret)
        if ret:
            ...
            # print(self.objp, corners2, self.cameraMatrix, self.distCoeffs)
            # res = cv2.solvePnPRansac(self.objp, corners2, self.cameraMatrix, self.distCoeffs,
            ...
            print(res)
            ret, b_rvecs, b_tvecs, inliers = res

            corners = corners2
            print()
            # rospy.loginfo(np.array2string(inliers))

        # print(b_rvecs, b_tvecs)
...
class TrackerFrame(wx.Frame):
    ...
    def __init__(self, trackedTopic, camera, node):
        ...
        self.node.get_logger().info('Selected topic for tracking: {}'.format(trackedTopic))
        self.node.get_logger().info('Image topic: {}, camera info topic: {}'.format(camera.imageTopic, camera.cinfoTopic))
        #rospack = rospkg.RosPack()
        #self.packagePath = rospack.get_path('cam_calib')
        self.packagePath = get_package_share_directory('cam_calib')
        self.camera = camera
        ...
        self.rvecsBuffer = []
        self.tvecsBuffer = []
        self.topicFreshnessCounter = self.node.get_clock().now()#rospy.Time.now()
...
#        self.imageView = ImageViewPanel(self, sampleCallback=self.onSampleColorReceived)
        ...
        self.markerIDs = np.zeros(CHECKERBOARD_SIZE, dtype=int)
        ...
        self.markerPublisher = self.node.create_publisher(Marker, '~/calibration_markers', qos_profile=QoSProfile(depth=1))
        self.trackedPoint = None

        self.tf_buffer = Buffer()
        self.tfListener = TransformListener(self.tf_buffer, self.node)        
        #self.posePublisher = rospy.Publisher(PoseStamped, '/end_point', queue_size=10)
        self.posePublisher = self.node.create_publisher(PoseStamped, '/end_point', qos_profile=QoSProfile(depth=1))
        ...
        tracking_qos = QoSProfile(depth=10, reliability=QoSReliabilityPolicy.RELIABLE)
        #self.trackingSubscriber = rospy.Subscriber(trackedTopic, rostopic.get_topic_class(trackedTopic)[0], self.trackedTopicCallback)
        self.trackingSubscriber = self.node.create_subscription(eval(self.node.get_publishers_info_by_topic(trackedTopic)[0].topic_type.replace('/', '.')),
                                                           trackedTopic, callback = self.trackedTopicCallback, qos_profile=tracking_qos)
        ...
        self.imageSubscriber = self.node.create_subscription(eval(self.node.get_publishers_info_by_topic(camera.imageTopic)[0].topic_type.replace('/', '.')),
                                                        camera.imageTopic, callback = self.updateImage, qos_profile=image_qos)

        #transmitter = tf.TransformBroadcaster()
        #self.transmitter = TransformBroadcaster(self)
        #poser = rospy.Publisher('/test_pose', PoseStamped, queue_size=10)
        #self.poser = self.create_publisher('/test_pose', PoseStamped, queue_size=10)
    @staticmethod
    ...
    def solvePosition(self):
        ...
        for i, (c, r) in enumerate(np.ndindex(CHECKERBOARD_SIZE[1], CHECKERBOARD_SIZE[0])):
            ...
            marker.header.stamp = self.node.get_clock().now().to_msg()  #rospy.Time.now()
            marker.id = int(self.markerIDs[r, c] + 10000)
            marker.text = "alfa"
            ...
            tpB = np.array(
                # [(np.matrix(tf.transformations.quaternion_matrix([rt.x, rt.y, rt.z, rt.w])).T * pB.reshape(4, 1))[:3] for (rt, pB) in zip(grotations, pBs)]
                [(np.matrix(quat2mat(quat)).T * pB.reshape(4, 1))[:3] for (quat, pB) in zip(grotations, pBs)]
        ...
        self.node.get_logger().info("Found {} inliers out of {} points total after plane fitting. Pose optimization will only use points {}.".format(len(inliers), B.shape[1], np.array2string(goodIdx)))
        ...
        self.node.get_logger().info('Calibration complete.')
        ...
        self.node.get_logger().info('Static translation from the tracked point to the center = {}'.format(self.trackedToTargetTranslation))
        ...
        quat = tf3.quaternions.mat2quat(R)
        ...
        self.transformStampedMessage.transform.rotation.x = quat[1]
        self.transformStampedMessage.transform.rotation.y = quat[2]
        self.transformStampedMessage.transform.rotation.z = quat[3]
        self.transformStampedMessage.transform.rotation.w = quat[0]
    ...
    def sendStaticTransformation(self, quat, t):
        ...
        static_transformStamped.header.stamp = self.node.get_clock().now().to_msg()  #rospy.Time.now()
    ...
    def sendTransformation(self):
        self.transformStampedMessage.header.stamp = self.node.get_clock().now().to_msg()  #rospy.Time.now()
        self.tfBroadcaster.sendTransform(self.transformStampedMessage)
    ...
    def trackedTopicCallback(self, msg):
        if self.phase & self.PHASE_WAITING_POSE_TOPIC:
            ...
            if (self.node.get_clock().now() - self.topicFreshnessCounter).to_msg().sec > 3:
                if (self.node.get_clock().now() - self.topicFreshnessCounter).to_msg().sec > 10:
...
#        img = cv2.cvtColor(img, cv2.COLOR_RGBA2RGB)
    ...
    def onKeyDown(self, key):
        ...
        if not self.phase & self.PHASE_POINT_GATHERING:
            ...
            if key == pynput.keyboard.Key.space:
                ...
                marker.header.frame_id = TRACKED_FRAME
                marker.header.stamp = self.node.get_clock().now().to_msg()  #rospy.Time.now()
                marker.id = int(self.markerIDs[r, c])
    ...
    def onClose(self, event):
        ...
        self.keyDaemon.stop()
        try:
            self.imageSubscriber.destroy()
            self.trackingSubscriber.destroy()
        except:
            pass  # ignore for now
        self.DestroyLater()
...
class Camera(object):
    ...
    def __init__(self, updateCam_cb: Callable, autoCalibrate:bool = False, node:Node = None):
        self.ID = uuid.uuid4()
        ...
        #thread.start()
        self.node = node
        # self.node = rclpy.create_node('foo')
    ...
    def setCinfoCB(self, cinfoTopic):
        ...
        self.node.get_logger().info('Subscribing to camera info on topic {}.'.format(cinfoTopic))
        #self.cinfoSubscriber = rospy.Subscriber(cinfoTopic, rostopic.get_topic_class(cinfoTopic)[0], self.__cameraInfo_cb)
        #self.cinfoSubscriber = self.create_subscription(cinfoTopic, rostopic.get_topic_class(cinfoTopic)[0], self.__cameraInfo_cb)
        qos = QoSProfile(depth=10, reliability=QoSReliabilityPolicy.BEST_EFFORT)
        self.cinfoSubscriber = self.node.create_subscription(eval(self.node.get_publishers_info_by_topic(cinfoTopic)[0].topic_type.replace('/', '.')),
                                                             cinfoTopic, self.__cameraInfo_cb, qos_profile = qos)
    ...
    def setImageCB(self, imageTopic):
        ...
        self.node.get_logger().info('Subscribing to images on topic {}.'.format(imageTopic))
        TopicType = eval(self.node.get_publishers_info_by_topic(imageTopic)[0].topic_type.replace('/', '.'))
        print(TopicType)
        print(type(TopicType))
        ...
        qos = QoSProfile(depth=10, reliability=QoSReliabilityPolicy.BEST_EFFORT)
        self.imageSubscriber = self.node.create_subscription(eval(self.node.get_publishers_info_by_topic(imageTopic)[0].topic_type.replace('/', '.')), imageTopic, #eval(self.node.get_publishers_info_by_topic(imageTopic)[0].topic_type.replace('/', '.')),
                                                             callback=self.image_cb, qos_profile = qos)
    ...
    def __cameraInfo_cb(self, msg):
        ...
        if self.isPoseCalibrated:
            if time.time() - self.last_time > 1:
                ...
                assert not np.any(np.isnan(rot_mat.T))
                quat = tf3.quaternions.mat2quat(rot_mat.T[:3,:3])
                # flip order of because of different convention in transforms3d and ROS
                rotation = np.hstack([quat[1:],quat[0]])
        ...
        if self.phase & self.PHASE_CAM_WAITING_CAM_INFO:
            ...
            self.boardPoseCalculator = PoseEstimator(self.camMatrix, self.distMatrix, self.node)
            wx.CallAfter(self.updateCam_cb, self, flag=self.UPDATE_CHECK_STATUS)
    ...
    def image_cb(self, msg):
        ...
        if self.phase == self.PHASE_CAM_EXT_CALIB:
            ...
            if ret:
                ...
                # Only add valid values to the buffer
                if not np.any(np.isnan(rvecs)) and not np.any(np.isnan(tvecs)):
...
class CameraPoseCalibration(wx.App):
    ...
    def OnInit(self):
        ...
        self.node = rclpy.create_node("pose_estimator")
        #rospack = rospkg.RosPack()
        #self.packagePath = rospack.get_path('cam_calib')
        self.packagePath = get_package_share_directory('cam_calib')
        self.cameras = []
...
#        self.whiteRect = ogrid
    ...
    def addCamera(self, imageTopic, cinfoTopic):
        try:
            camera = Camera(self.updateCam, node=self.node)
    ...
    def loadCameras(self, fileName):
        with open(os.path.join(self.packagePath, 'data', fileName), 'r') as f:
            try:
                ...
                data = crow_config.load_yaml_config(f)
        ...
        if type(obj.GetParent()) is wx.ToolBar:
            if obj.Name == 'buttonAddCam':
                with wx.Dialog(self.frame, title='Select image and camera info topics') as dlg:
                    ...
                    imageList, imageTopicEdit = getTopicListBAndEdit('sensor_msgs/msg/Image', dlg)
                    cinfoList, cinfoTopicEdit = getTopicListBAndEdit('sensor_msgs/msg/CameraInfo', dlg)
                ...
                self.node.get_logger().info('Selected image topic: {} and camera info topic: {}'.format(imageTopic, cinfoTopic))
                ...
                with open(os.path.join(self.packagePath, 'data', fname), 'w') as f:
                    ...
                    crow_config.dump_yaml_file(f, d)
                ...
                try:
                    ...
                    with open(last_file, 'r+') as f:
                        data = crow_config.load_yaml_config(f)	#rospy.client.yaml.load(f)
                        if data:
                            n = len(data)
                        else:
                            n = 0
                        #rospy.client.yaml.dump({'save' + str(n): {'time': stamp, 'file': fname}}, stream=f, default_flow_style=False)
                        crow_config.dump_yaml_file(f, {'save' + str(n): {'time': stamp, 'file': fname}})
                except IOError:
                ...
                with wx.Dialog(self.frame, title='Select topic for tracking') as dlg:
                    ...
                    topicList, topicEdit = getTopicListBAndEdit(['geometry_msgs/msg/PoseStamped', 'geometry_msgs/msg/TransformStamped', 'geometry_msgs/msg/Pose'], dlg)
                    ...
                    if dlg.ShowModal() == wx.ID_OK:
                        ...
                        print('No topics selected, aborting')
                        return
                trackingFrame = TrackerFrame(topic, camera, self.node)
            ...
            if obj.Name == 'buttonImageTopic':
                ...
                self.node.get_logger().info('Selected image topic: {}'.format(imageTopic))
                ...
                self.node.get_logger().info('Selected camera info topic: {}'.format(cinfoTopic))
                camera.setCinfoCB(cinfoTopic)
    ...
    def onToggle(self, event):
        ...
        if obj.Name == 'toggleActive':
            if not obj.GetValue():
                try:
                    camera.imageSubscriber.destroy()
                    #rospy.loginfo('Unregisterd from image topic {}'.format(camera.imageTopic))
                    self.node.get_logger().info('Unregisterd from image topic {}'.format(camera.imageTopic))
