...
from crow_particle_filter.filters import ParticleFilter

# Tracker
from crow_particle_filter.tracker import Tracker
from crow_particle_filter.tracker.tracker_avatar import Avatar
from crow_particle_filter.tracker.tracker_config import DETECTIONS_FOR_SETUP_NEEDED
from crow_particle_filter.utils import ftl_pcl2numpy, ftl_numpy2pcl
...
class ParticleFilterNode(Node):
    ...
    def __init__(self, node_name="particle_filter"):
        ...
        qos = QoSProfile(depth=30, reliability=QoSReliabilityPolicy.BEST_EFFORT)
    ...
    def update_particle_filter(self, particle_filter, tracking_on=True, pose_est_on=True):
        ...
        if particle_filter.n_models > 0:
            ...
            pcls = []
            for i, np_pcl in enumerate(pcl_points):  # clear empty PCLs
                if len(np_pcl) > 0:
