...
class ParticleFilter():
    ...
    def getPclsEstimates(self):
        ...
        for k, v in self.model_stored_pcls.items():
            ...
            ests_pcls.append(v_pcls)# -1*min(self.NUM_STORED_PCLS+1, len(v)+1)])
    ...
    def _estimate_classes(self):
        ...
        # cls_estimate = mode(self.model_class_history, axis=1, nan_policy="omit")[0]
        cls_estimate = mode(self.model_class_history, axis=1, nan_policy="omit")[0]
        cls_estimate[np.logical_not(np.isfinite(cls_estimate))] = -1
    ...
    def _processMeasurements(self):
        ...
        """
        # helper variables
        self._model_pcls = np.empty(self.n_models, dtype=object)
        self._model_trees = np.empty(self.n_models, dtype=object)
        ...
        for idx, mpcls in enumerate(self._model_pcls):
            if mpcls is None:
                continue
            pcls = np.empty((0, 3), dtype=float)
            pcl_centers = np.empty((0, 3), dtype=float)
            pcl_dimensions = np.empty((0, 3), dtype=float)
            labels = []
            ...
            self.model_class_history[idx, -1] = mode(labels)[0]
    ...
    def _append_pcl_to_model(self, model_idx, pcl, est_class, score, pcl_center=None, pcl_dimension=None):
        ...
        if self.model_stored_pcls.get(str(self.model_uuid[model_idx])) is None:
            self.model_stored_pcls[str(self.model_uuid[model_idx])] = {}
            self.model_stored_pcls[str(self.model_uuid[model_idx])]["pcl"] = pcl
            self.model_stored_pcls[str(self.model_uuid[model_idx])]["label"] = [est_class]
        else:
            self.model_stored_pcls[str(self.model_uuid[model_idx])]["pcl"] = np.vstack([pcl, self.model_stored_pcls[str(self.model_uuid[model_idx])]["pcl"]])
            self.model_stored_pcls[str(self.model_uuid[model_idx])]["label"].append(est_class)
