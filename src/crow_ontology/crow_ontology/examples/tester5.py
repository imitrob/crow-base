...
class OntoTester(Node):
    ...
    def test_color(self, ot):
        print("Color: ")
        print([self.crowracle.get_nlp_from_uri(c)[0] for c in self.crowracle.getColors()])

        soc = SceneOntologyClient(ot)
        print(f"Object with idx: {[(o.name, o.color) for o in soc.get_scene2().objects]}")
        
        qres = self.crowracle.getTangibleObjects()
        
        print("Object to change: ", qres[0])
        self.crowracle.update_color(qres[0], 'cyan')
        
        print(f"[after color change]")
        print(f"Object with idx: {[(o.name, o.color) for o in soc.get_scene2().objects]}")

        self.crowracle.update_color(qres[0], 'gold')

        print(f"[after second color change]")
        print(f"Object with idx: {[(o.name, o.color) for o in soc.get_scene2().objects]}")
