...
def ftl_pcl2numpy(pcl_msg, uses_stupid_bgr_format=True):
    ...
    dwordData = (
        np.frombuffer(pcl_msg.data, dtype=np.byte).view(np.float32).reshape(-1, step)
    )  # organize the bytes into double words (4 bytes)
    xyz = dwordData[:, :3]  # assuming x, y, z are the first three dwords
    if "rgb" in [f.name for f in pcl_msg.fields]:
        rgb_offset = int(
            next((f.offset for f in pcl_msg.fields if f.name == "rgb")) / 4
        )  # rgb offset in dwords
        rgb_raw = dwordData[:, rgb_offset]  # extract rgb data
        rgb3d = (
            rgb_raw.view(np.uint32)
            .copy()
            .view(np.uint8)
            .reshape(-1, 4)[
                :, slice(1, None) if pcl_msg.is_bigendian else slice(None, 3)
            ]
        )
...
def ftl_numpy2pcl(xyz, orig_header, rgb=None):
    ...
    if rgb is not None:
        ...
        fields = [
            ...
            for i, n in enumerate(list("xyz") + ["rgb"])
        ]
        fields[-1].offset = 16
        k = 5
        dataa = np.concatenate(
            (xyz.T, np.zeros((num_points, 1), dtype=np.float32), rgb[:, np.newaxis]),
            axis=1,
        )
    else:
        fields = [
