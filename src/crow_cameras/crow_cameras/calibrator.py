...
from crow_cameras.utils.tf_utils import (getTransformFromTF, make_quaternion,
                                         make_vector3)
...
class Calibrator(Node):
    ...
    def __init__(self, node_name="calibrator"):
        ...
        config_file = crow_config.get_config_file(
            "crow_cameras/camera_transformation_data.yaml"
        )

        cgtfg = CameraGlobalTFGetter(config_file)
        g2r_tf = cgtfg.get_camera_transformation("global_2_robot", cast=True)
        ...
        # DECLARE PARAMS
        imageTopicsParamDesc = ParameterDescriptor(type=int(ParameterType.PARAMETER_STRING_ARRAY), description='List of available image topics', dynamic_typing=True)
        # imageTopicsParamDesc = ParameterDescriptor(description='List of available image topics')
        self.declare_parameter("image_topics", value=[], descriptor=imageTopicsParamDesc)
        infoTopicsParamDesc = ParameterDescriptor(type=int(ParameterType.PARAMETER_STRING_ARRAY), description='List of camera_info topics for the available cameras', dynamic_typing=True)
        # infoTopicsParamDesc = ParameterDescriptor(description='List of camera_info topics for the available cameras')
        self.declare_parameter("info_topics", value=[], descriptor=infoTopicsParamDesc)
        cameraNodesParamDesc = ParameterDescriptor(type=int(ParameterType.PARAMETER_STRING_ARRAY), description='List of available camera nodes (e.g. "/camera<X>/camera")', dynamic_typing=True)
        # cameraNodesParamDesc = ParameterDescriptor(description='List of available camera nodes (e.g. "/camera<X>/camera")')
        self.declare_parameter("camera_nodes", value=[], descriptor=cameraNodesParamDesc)
        cameraSerialsParamDesc = ParameterDescriptor(type=int(ParameterType.PARAMETER_STRING_ARRAY), description='List of serial numbers of available cameras', dynamic_typing=True)
        # cameraSerialsParamDesc = ParameterDescriptor(description='List of serial numbers of available cameras')
        self.declare_parameter("camera_serials", value=[], descriptor=cameraSerialsParamDesc)
        cameraNamespacesParamDesc = ParameterDescriptor(type=int(ParameterType.PARAMETER_STRING_ARRAY), description='List of namespaces (e.g. "/camera<X>") of available cameras', dynamic_typing=True)
        # cameraNamespacesParamDesc = ParameterDescriptor(description='List of namespaces (e.g. "/camera<X>") of available cameras')
        self.declare_parameter("camera_namespaces", value=[], descriptor=cameraNamespacesParamDesc)
        cameraMatricesParamDesc = ParameterDescriptor(type=int(ParameterType.PARAMETER_STRING_ARRAY), description='List of camera intrinsic parameters for available cameras', dynamic_typing=True)
        # cameraMatricesParamDesc = ParameterDescriptor(description='List of camera intrinsic parameters for available cameras')
        self.declare_parameter("camera_intrinsics", value=[], descriptor=cameraMatricesParamDesc)
        extrinsicsParamDesc = ParameterDescriptor(type=int(ParameterType.PARAMETER_STRING_ARRAY), description='Depth to color and color to global transformations for cameras.', dynamic_typing=True)
        # extrinsicsParamDesc = ParameterDescriptor(description='Depth to color and color to global transformations for cameras.')
        self.declare_parameter("camera_extrinsics", value=[], descriptor=extrinsicsParamDesc)
        cameraFramesParamDesc = ParameterDescriptor(type=int(ParameterType.PARAMETER_STRING_ARRAY), description='List of camera coordinate frames for available cameras', dynamic_typing=True)
        # cameraFramesParamDesc = ParameterDescriptor(description='List of camera coordinate frames for available cameras')
        self.declare_parameter("camera_frames", value=[], descriptor=cameraFramesParamDesc)
        # globalFrameParamDesc = ParameterDescriptor(type=ParameterType.PARAMETER_STRING, description='The global workspace frame name.')
        globalFrameParamDesc = ParameterDescriptor(description='The global workspace frame name.')
        self.declare_parameter("global_frame_id", value=self.GLOBAL_FRAME_NAME, descriptor=globalFrameParamDesc)
        # globalTFParamDesc = ParameterDescriptor(type=ParameterType.PARAMETER_DOUBLE_ARRAY, description='Transformation from robot to global frame.')
        globalTFParamDesc = ParameterDescriptor(description='Transformation from robot to global frame.')
        self.declare_parameter("robot2global_tf", value=r2g_mat.ravel().tolist(), descriptor=globalTFParamDesc)

        # parse args
        # self.get_logger().warn(f">>> ALL ARGS:\n{sys.argv}")
        ...
        self.tf_static_broadcaster = tf2_ros.static_transform_broadcaster.StaticTransformBroadcaster(self)
    ...
    def timer_cb(self):
        ...
        new_found_cameras = [c for c in online_cameras if c not in self.cameras]
        print(f"Found cameras: {new_found_cameras}")

        for node_name, namespace in new_found_cameras:
            try:
            ...
            # 'camera4_color_optical_frame', 'camera4_depth_optical_frame', 'camera4_link'
            # print(f"Looking for frames: {optical_frame, depth_frame, link_frame}")
            # retrieve depth to color transformation

            self.get_logger().info("Creating futures")
            self.get_logger().info(f"Can transform {self.tf_buffer.can_transform(link_frame, optical_frame, self.get_clock().now(), rclpy.duration.Duration(seconds=5))}")
            ...
            self.get_logger().info(f"Can transform {self.tf_buffer.can_transform(link_frame, optical_frame, self.get_clock().now(), rclpy.duration.Duration(seconds=5))}")
            self.get_logger().info(f"self.optical_frames: {str(self.optical_frames)}")
            self.get_logger().info(f"link_frame: {str(link_frame)}")
            self.get_logger().info(f"optical_frame: {str(optical_frame)}")
        ...
        if len(self.cameras) == len(self.camera_serials):
    ...
    def found_dtc_transform_cb(self, future, camera_ns):  # depth to color
        ...
        self.get_logger().info("DTC to CB transform called")
    ...
    def found_ctl_transform_cb(self, future, camera_ns):  # color to baselink
        ...
        self.get_logger().info("CTL to CB transform called")
    ...
    def camera_info_cb(self, msg, camera_ns):
        if camera_ns not in self.tf_depth_to_color or camera_ns not in self.tf_color_to_global:  # wating to get the transforms first
            #self.get_logger().info(f"Waiting for transforms: {self.tf_depth_to_color}, {self.tf_color_to_global}")
            return
        ...
        # print("------------------------------------")
        # print(self.camera_serials)
        # print(self.camera_namespaces)
        # print(camera_ns)
        ...
        paramCameraExtrinsics.append(json.dumps({
            ...
            "ctg_tf": np.linalg.inv(self.tf_color_to_global[camera_ns]).astype(np.float32).tolist(),
            # FIXME: remove the need for ivnersion!!! "ctg_tf": self.tf_color_to_global[camera_ns].astype(np.float32).tolist(),
        }))
        ...
        print('-------------------')
        print(self.get_parameter("camera_frames").get_parameter_value().string_array_value)
