...
def my_bnode_ext(node):
    ...
    elif isinstance(node, str) and not isinstance(node, URIRef):
        return Literal(node).n3()
...
class OntologyDatabase:
    ...
    def mergeFileIntoDB(self, filepath: str):
        ...
        """
        tmpGraph = Graph(identifier=self.identifier)
    ...
    def __len__(self):
        ...
        try:
            ...
            traceback.print_exc()
            raise RuntimeError("An attribute error occured while trying to get the legnth of the ontology.\n" \
                "This is most likely caused by an error code returned instead of the length.\n" \
                    "Checked that Fuseki is acutally running (e.g., port can be blocked by another process: 'netstat -nap | grep <fuseki_port>')")
        return length
